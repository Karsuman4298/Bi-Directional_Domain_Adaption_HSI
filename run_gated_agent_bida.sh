...
echo "=================================================================================="
echo " Training: GatedAgentBiDA (4 Seeds: ${SEEDS[*]})"
echo "=================================================================================="
...
run_task() {
    ...
    for SEED in "${SEEDS[@]}"; do
        ...
        OUTPUT=$(python3 train_gated_agent_bida.py \
            --source_name $SOURCE \
            --target_name $TARGET \
            --epoch $EPOCHS \
            --seed $SEED \
            ...
            --gate_init_bias $GATE_INIT_BIAS 2>&1)
    ...
    # Print the aggregated row for this task
    printf "%-15s | %-20s | %-20s | %-20s\n" "$TASK_NAME" "$OA_STATS" "$AA_STATS" "$KAPPA_STATS"
}

# Run the tasks
run_task "Houston13" "Houston18"
run_task "Houston18" "Houston13"
