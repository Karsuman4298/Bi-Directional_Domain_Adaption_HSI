...
from models.gated_agent_bida import GatedAgentBiDA
...
if __name__ == "__main__":
    ...
    # GatedAgentBiDA specific args
    parser.add_argument('--gate_hidden_ratio', type=float, default=0.25,
                        help='gate_hidden_ratio')
    parser.add_argument('--gate_init_bias', type=float, default=0.0,
                        help='gate_init_bias')
    parser.add_argument('--gate_reg_weight', type=float, default=0.0,
                        help='gate_reg_weight')
    ...
    if opts.debug_shapes:
        ...
        model = GatedAgentBiDA(opts.target_name, opts).to(device)
        model.eval()
    ...
    # Load AgentBiDA
    model = GatedAgentBiDA(opts.target_name, opts)
    model_ema = GatedAgentBiDA(opts.target_name, opts)
    ...
    model_dir = "./checkpoints/gated_agent_bida/" + opts.source_name + 'to' + opts.target_name + f"_agents{opts.num_agents}_heads{opts.num_heads}"
