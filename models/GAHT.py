...
class MyTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.pad(x).squeeze(dim=1)  # torch.Size([128, 64, 13, 13])
        B = x.shape[0]
        ...
        for i in range(self.num_stages):
            ...
            x, s = patch_embed(x)  # s = feature map size after patch embedding  torch.Size([128, 64, 13, 13])   torrch.Size([128, 256, 13, 13])
            for blk in block:  # torch.Size([128, 169, 256])
                x = blk(x)
        ...
        return x  # torch.Size([128, 169, 64])
    
    def forward(self, x):
        x = self.forward_features(x)  # torch.Size([128, 1, 64, 13, 13])
        x = x.mean(dim=1)
        x = self.head(x)
        return x
