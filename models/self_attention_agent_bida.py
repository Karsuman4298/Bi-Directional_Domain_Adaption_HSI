...
class AgentAttention(nn.Module):
    ...
    def forward(self, x, x2, inference_target_only=False, debug_shapes=False):
        ...
        if inference_target_only:
            ...
            # Standard self-attention for target branch during inference
            attn2 = (q2 @ k2.transpose(-2, -1)) * self.scale
            attn2 = attn2.softmax(dim=-1)
            attn2 = self.attn_drop(attn2)
            x2_out = (attn2 @ v2).transpose(1, 2).reshape(B, N, C)
            ...
            # Generate agents ONLY for cross-domain fusion branch
            ...
            # Standard Self-Attention for Source
            attn = (q @ k.transpose(-2, -1)) * self.scale
            attn = attn.softmax(dim=-1)
            attn = self.attn_drop(attn)
            x_out = (attn @ v).transpose(1, 2).reshape(B, N, C)

            # Standard Self-Attention for Target
            attn2 = (q2 @ k2.transpose(-2, -1)) * self.scale
            attn2 = attn2.softmax(dim=-1)
            attn2 = self.attn_drop(attn2)
            x2_out = (attn2 @ v2).transpose(1, 2).reshape(B, N, C)

            # Agent Attention for Cross-Domain Fusion
