...
class Attention_triple_branches(nn.Module):
    def __init__(self, dim, num_heads=8, agent_num=2, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        ...
        self.agent_tokens = nn.Parameter(torch.zeros(1, agent_num, dim))
        torch.nn.init.normal_(self.agent_tokens, std=.02)
    ...
    def forward(self, x, x2, inference_target_only=False):
        B, N, C = x2.shape
        if inference_target_only:
            agent_x2 = self.agent_tokens.expand(B, -1, -1).contiguous()
            agent_qkv2 = self.qkv(agent_x2).reshape(B, self.agent_num, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            agent_q2, agent_k2, _ = agent_qkv2[0], agent_qkv2[1], agent_qkv2[2]
            ...
            agent_features2 = attn_agent2 @ v2

            # Stage B
            attn_feature2 = (q2 @ agent_k2.transpose(-2, -1)) * self.scale
            attn_feature2 = attn_feature2.softmax(dim=-1)
            attn_feature2 = self.attn_drop(attn_feature2)
            x2_out = (attn_feature2 @ agent_features2).transpose(1, 2).reshape(B, N, C)
            
            x2 = self.proj(x2_out)
            x2 = self.proj_drop(x2)
            x, x3, x4 = None, None, None
        else:
            agent_x = self.agent_tokens.expand(B, -1, -1).contiguous()
            agent_qkv = self.qkv(agent_x).reshape(B, self.agent_num, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            agent_q, agent_k, _ = agent_qkv[0], agent_qkv[1], agent_qkv[2]
            ...
            agent_features_st = attn_agent_st @ v_st

            # Stage B
            attn_feature = (q @ agent_k.transpose(-2, -1)) * self.scale
            attn_feature = attn_feature.softmax(dim=-1)
            attn_feature = self.attn_drop(attn_feature)
            x_out = (attn_feature @ agent_features).transpose(1, 2).reshape(B, N, C)
            ...
            attn_feature_st = (q_st @ agent_k_st.transpose(-2, -1)) * self.scale
            attn_feature_st = attn_feature_st.softmax(dim=-1)
            attn_feature_st = self.attn_drop(attn_feature_st)
            x_st_out = (attn_feature_st @ agent_features_st).transpose(1, 2).reshape(2*B, N, C)
...
if __name__ == '__main__':
    ...
    t_tar = torch.randn(size=(3, 1, 48, 13, 13))
    print("input shape:", t.shape)
    net = BiDA(dataset='Houston13', opts=Opts())
    out = net(t, t_tar)
    print("output shapes (train):", [o.shape for o in out])
    
    net.eval()
    out_eval = net(t, t_tar, inference_target_only=True)
    print("output shape (eval):", out_eval[1].shape)
