...
class SSFTTnet(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = self.transformer(x, mask)  # main game torch.Size([128, 5, 64])
        x = self.to_cls_token(x[:, 0])  # torch.Size([128, 64])
        x = self.nn1(x)  # torch.Size([128, 5])
        return x
...
if __name__ == '__main__':
    ...
    net = ssftt(dataset='sa', patch_size=7)
