...
def prototype_calc(src_spa, src_y, model, Classes):
    ...
    for c in range(Classes):
        ...
        if number * 100 < num_src:
            temp1_1 = src_spa_c[number * 100:num_src, :, :, :]
            temp1_1 = temp1_1.cuda()
            with torch.no_grad():
                temp_2_1, _, _ = model(temp1_1)
            temp_2_1 = temp_2_1.flatten(2)
...
def train_network(train_sd_loader, train_patch_src, train_y_src, train_td_loader, test_loader, LR, EPOCH, l1, Classes,
                  dataset, factor_lambda):
    cnn = Network(l1=l1, Classes=Classes)
    dis = Discriminator()
    cnn.cuda()
    dis.cuda()
    ...
    for epoch in range(next_epoch if next_epoch is not None else 0, EPOCH):
        ...
        for step, ((b_x1, b_y), b_u1) in enumerate(
                zip(train_sd_loader, train_td_loader)):
            b_x1, b_y = b_x1.cuda(), b_y.cuda()  # Move data to GPU
            b_u1 = b_u1[0].cuda()
            encode_src, cls_src, mapping_src = cnn(b_x1)  # Model output according to labelled train set
            encode_tar, cls_tar, mapping_tar = cnn(b_u1)
            dis.zero_grad()
